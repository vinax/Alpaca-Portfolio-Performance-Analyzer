...
st.write("Disclaimer: This software is for educational purposes only. It is provided as-is and is not intended for commercial use without explicit permission. It is not financial advice or a trading recommendation. Use at your own risk. The author is not responsible for financial losses.\n\nSecurity Warning: Ensure your API Key and Secret are kept safe and do not auto-save to browser.")
...
if is_api_key_valid()and is_api_connection_valid(api):
...
if is_api_key_valid()and is_api_connection_valid(api):
    ...
    try: # Create and process dataframes for portfolio and activities
        ...
        if portfolio_df is not None and activities_df is not None: # Merge DataFrames
            ...
            merged_df = None
        if merged_df is not None: # Aggregate merged data
    ...
    def plot_combined_cumulative_chart(portfolio_df, spy_df):
        try:
            ...
            plt.plot(
                spy_df['Date'],
                spy_df['% Return'],  # Convert to percentage
                label='SPY Returns (%)',
                color='green'
            )
            plt.xlabel('Date')
